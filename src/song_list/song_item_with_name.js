...
const SongItemWithName = (
    {
        song = { name: 'Honeybee', artists: 'Unknown Mortal Orchestra', image: 'umo.png', user: 'test'}
    }
) => {
    return(
        <div className="p-1 ps-0">
            <p><Link to={`/profile/${song.user._id}`}>
                {song.user.username}
            </Link>{song.user.role == "ARTIST" ? " has claimed:" : " recommends"}</p>
